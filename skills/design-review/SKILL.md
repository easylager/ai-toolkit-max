...
- Run the Anti-Slop Review (`rules/frontend/design.md`) against the rendered UI, not just the prototype — unexplained purple gradients, glassmorphism, floating cards, glowing blobs, default-Tailwind-looking layouts, decorative 3D, motion with no purpose. If found and not justified in the Art Direction, flag it — don't silently wave it through, and don't accept "add more effects" as the fix.
