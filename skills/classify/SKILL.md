...
- Reference only existing skill names in the recommended chain (`task`, `clarify`, `design`, `plan`, `impact`, `estimate`, `challenge`, `next`, `verify`, `design-review`, `status`, `reconcile`, `test`, `review`, `debug`, `short`) — never invent a new one. `implement` may also appear in the chain, but it is not a toolkit skill: it denotes Claude's native code-writing behavior and has no SKILL.md.
